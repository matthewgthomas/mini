...
plt_weekdays <- 
  ...
  geom_point(data = player_times_weekday_summary, aes(y = mean_seconds, text = str_glue("{Player}'s average time to complete Minis on {Weekday}s is {mean_seconds_string}")), shape = 4, size = 3, position = position_jitterdodge(), show.legend = FALSE) +
  coord_flip() +
...
#TODO: Which setters are we each fastest, slowest and most variable with?
```

```{r}
#| label: Spread of completion times by setter
#| echo: false
#| message: false
#| warning: false
player_times_setter |> 
  ggplot(aes(x = seconds, y = Setter)) +
  geom_density_ridges(aes(colour = Setter, fill = Setter), scale = 4, show.legend = FALSE) + 
