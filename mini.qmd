...
<h1 style="margin-bottom: 0;">Matt & Tam's</h1>

:::::::: crossword-grid
::: {.letter-cell style="margin-bottom: 2rem; margin-top: 0;"}
M
:::
...
The chart below shows the spread of our completion times on each day of the week:
...
player_times_weekday |> 
  ...
  geom_density_ridges(aes(colour = Weekday, fill = Weekday), scale = 4, show.legend = FALSE) + 
...
Tam does best when **`r best_setter_tam`** sets the Mini, winning `r best_setter_tam_wins` of `r best_setter_tam_games` games.

Matt's best Mini setter is **`r best_setter_matt`**, with `r best_setter_matt_wins` wins out of `r best_setter_matt_games` games.
...
Matt tends to be fastest when **`r matt_fastest_setter$Setter`** sets the Mini, with an average completion time of **`r matt_fastest_setter$mean_seconds_string`**. But he's slowest when **`r matt_slowest_setter$Setter`** is the setter, completing these Minis in **`r matt_slowest_setter$mean_seconds_string`** on average.

Tam is speediest when **`r tam_fastest_setter$Setter`** sets the Mini, gunning it in **`r tam_fastest_setter$mean_seconds_string`**. She's slowest when they're set by **`r tam_slowest_setter$Setter`**, with an average completion time of **`r tam_slowest_setter$mean_seconds_string`**.
